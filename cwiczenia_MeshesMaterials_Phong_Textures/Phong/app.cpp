...
void SimpleShapeApplication::init() {
    ...
    xe::PhongMaterial::init();  //new Phong material added
    ...
    num_of_lights++;    //increase light count

    glGenBuffers(1, &pvm_buffer_handle);    //new buffer
    glBindBuffer(GL_UNIFORM_BUFFER, pvm_buffer_handle);
    glBufferData(GL_UNIFORM_BUFFER, 2 * sizeof(glm::mat4) + sizeof(glm::mat4x3), nullptr, GL_STATIC_DRAW);  //GL_STATIC_DRAW usage pattern of the data store
    glBindBufferBase(GL_UNIFORM_BUFFER, 1, pvm_buffer_handle);
    glBindBuffer(GL_UNIFORM_BUFFER, 0);

    glGenBuffers(1, &light_buffer);
    glBindBuffer(GL_UNIFORM_BUFFER, light_buffer);
    glBufferData(GL_UNIFORM_BUFFER, sizeof(glm::vec3) + sizeof(glm::uint) + sizeof(xe::PointLight) * num_of_lights, nullptr, GL_STATIC_DRAW);
